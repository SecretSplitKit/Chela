...
| SHA-256 in the engine | identifier + per-share checksum | **gone** — SHA-256 remains only in `chela-bip39` (mnemonic checksum) |
...
with probability `1/2048`; for seeds the BIP-39 mnemonic checksum is the backstop (§5).

### 3.3 Y values — SSS-split body (words 2 .. W-2)

The body is the secret payload with the **kind byte appended**, then Shamir-split. The Y values
are this share's SSS output bytes for its `x`, length `== body_len`, packed MSB-first per byte,
11 bits at a time, final word zero-padded.
...
8. Split `body` into `payload ‖ kind_byte` (kind is the last byte); validate `kind_byte` is in
   the table and `payload` length fits the kind.
9. Interpret. For BIP-39, re-encode entropy → mnemonic; its built-in checksum is the integrity
   backstop should a `1/2048` nonce collision have let a wrong/foreign subset through. Text has
   no inherent backstop (accepted, §7).

No identifier-driven kind search, no SHA anywhere in recovery. A decoder MUST accept a bare word
list; the card label and JSON metadata are advisory.
...
- **Generation binding is 11-bit.** Two unrelated generations collide on the nonce with
  probability `1/2048`. A collision plus a mixed subset would pass step 6; the BIP-39 checksum
  (seeds) is the backstop, and text has none. Stronger binding would cost more words; one word
  was the call. Note the upside: because the id is per-generation, even re-splitting the *same*
  secret is correctly refused — which a hash-of-secret id would have wrongly accepted.
...
- **Size:** +1 word (nonce) and, when the appended kind byte crosses an 11-bit boundary, +1
  more. A 12-word seed share goes 14 → 16 words; a 24-word seed share 26 → 27. Bought for
  words-alone recovery, a hidden kind, and in-band generation binding.
...
- **Identifier:** an 11-bit random nonce per generation, in word 1 — binds a generation, not a
  secret, and removes SHA-256 from the engine entirely.
