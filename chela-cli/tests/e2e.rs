...
fn round_trip_words_only_no_headers_2_of_3() {
    ...
    // one share per line. The words carry x, M, and the recovery set id on their own.
    let mnemonic =
        "abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon \
         abandon about";
    let shares = split_with_args(&["split", "--mnemonic", mnemonic, "-m", "2", "-n", "3"]);
...
fn mixed_shares_from_different_splits_rejected() {
    ...
    // One share from each split. v2 draws a fresh random recovery set id (a nonce) per generation, so
    // the two splits carry different recovery set ids; recover_secret sees the disagreement and
...
/// Extract the share coordinate `x` from a `CHELA-<RSID>-<x>-<M>-<N>-<W>` header
...
fn split_writes_all_four_outputs_when_all_flags_given() {
    ...
    // And the bundle filename includes the recovery set id.
