...
///
/// ```json
/// { "ok": true, "shares": [{ "x": 17, "threshold": 3, "total": 5, "recovery_set_id": "05A3",
///     "card_code": "CHELA-05A3-17-3-5-16", "words": ["..."] }, ...] }
/// ```
///
/// `total` is present only when the split knows it; `recovery_set_id` is the 11-bit recovery set id in
/// 4 hex digits.
///
/// or on error:
///
...
pub(crate) fn do_split(input: &[u8]) -> Result<String, String> {
    ...
    for (i, share) in shares.iter().enumerate() {
        ...
        let recovery_set_id = format!("{:04X}", share.recovery_set_id & 0x7FF);
        let _ = write!(out, ",\"recovery_set_id\":{}", json::str(&recovery_set_id));
...
///      "recovery_set_id": "0058", "card_code": "CHELA-0058-9-3-5-16",
...
mod tests {
    ...
    #[test]
    fn split_json_uses_recovery_set_id_and_keeps_total() {
        let split_json = do_split(&build_split_text(2, 3, "alpha")).unwrap();
        // v2: recovery set id is the 4-hex recovery set id; the old `identifier` key is gone.
        assert!(
            split_json.contains("\"recovery_set_id\":\""),
            "got: {split_json}"
        );
        assert!(!split_json.contains("\"identifier\""));
        // The split knows N, so `total` is present.
        assert!(split_json.contains("\"total\":3"));
        // recovery set id must match the recovery set id printed in the card header.
        let cards = parse_split_cards(&split_json);
        let recovery_set_id = cards[0].0.split('-').nth(1).unwrap();
        assert!(split_json.contains(&format!("\"recovery_set_id\":\"{recovery_set_id}\"")));
    }
