...
pub enum ImportError {
    ...
    WordCountMismatch,
    /// `recovery_set_id` isn't a 4-character ASCII hex string.
    BadRecoverySetId,
    /// The advisory `card_number` / `threshold` / `total` / `recovery_set_id` disagree with the
...
fn decode_share_value(v: &Value) -> Result<Share, ImportError> {
    ...
    // Words are authoritative: x, M, and the recovery set id come from them, verified by the CRC.
    let words_arr = v
        .get("words")
        .and_then(Value::as_array)
    ...
    if let Some(recovery_set_id) = v.get("recovery_set_id").and_then(Value::as_str) {
        let recovery_set_id = parse_recovery_set_id(recovery_set_id)?;
        if recovery_set_id != d.recovery_set_id {
    ...
    Ok(Share {
        ...
        recovery_set_id: d.recovery_set_id,
        total,
        kind,
        word_indices,
    })
...
/// Parse a 4-hex-char `recovery_set_id` like `"3058"` into the 11-bit recovery set id. Case-insensitive.
fn parse_recovery_set_id(s: &str) -> Result<u16, ImportError> {
    if s.len() != 4 || !s.is_ascii() {
        return Err(ImportError::BadRecoverySetId);
    }
    let n = u16::from_str_radix(s, 16).map_err(|_| ImportError::BadRecoverySetId)?;
    // The recovery set id is 11 bits; a well-formed recovery set id has its top 5 bits clear. Reject rather than
...
impl core::fmt::Display for ImportError {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            ...
            Self::BadRecoverySetId => f.write_str("recovery set id is not 4 hex characters"),
            Self::BadThresholdTotalOrIndex => f.write_str(
                "advisory recovery set id / card_number / threshold disagree with the words",
            ),
...
mod tests {
    ...
    #[test]
    fn json_recovery_set_id_above_11_bits_rejected() {
        let s = sample();
        let real = alloc::format!("\"recovery_set_id\":\"{:04X}\"", s.recovery_set_id & 0x7FF);
        // 0xF800 has bits above the 11-bit recovery set id range; it must be rejected, not masked.
        let json = share_json(&s).replace(&real, "\"recovery_set_id\":\"F800\"");
        assert_eq!(decode_share_json(&json), Err(ImportError::BadRecoverySetId));
    }
    ...
    #[test]
    fn advisory_recovery_set_id_mismatch_rejected() {
        let s = sample();
        let real = alloc::format!("\"recovery_set_id\":\"{:04X}\"", s.recovery_set_id & 0x7FF);
        let wrong = alloc::format!(
            "\"recovery_set_id\":\"{:04X}\"",
            (s.recovery_set_id ^ 1) & 0x7FF
        );
    ...
    #[test]
    fn bad_recovery_set_id_rejected() {
        let s = sample();
        let real = alloc::format!("\"recovery_set_id\":\"{:04X}\"", s.recovery_set_id & 0x7FF);
        let json = share_json(&s).replace(&real, "\"recovery_set_id\":\"ZZZZ\"");
        let html = wrap_block(&json);
        let result = extract_shares_from_html(&html).unwrap();
        assert!(matches!(result[0], Err(ImportError::BadRecoverySetId)));
    }
