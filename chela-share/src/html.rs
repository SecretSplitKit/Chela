...
fn render_share_page(
    ...
    let id = format!("{:04X}", share.recovery_set_id & 0x7FF);
    ...
    // Recovery pointer only - detailed instructions live in the repository's README so
    // they can be updated without re-printing shares.
    out.push_str("  <footer class=\"recovery\">\n");
    out.push_str("    <h2>How to recover the secret</h2>\n");
    write!(
        out,
        "    <p>Gather <strong>{}</strong> of the <strong>{}</strong> shares from set <code>{id_esc}</code>, then follow the recovery instructions at:</p>\n",
        share.threshold, total,
    )
    .expect("write to String");
    out.push_str("    <p class=\"recovery-url\"><strong>https://github.com/SecretSplitKit/Chela</strong></p>\n");
...
///   "recovery_set_id": "3058",              // 4-hex generation recovery set id
...
mod tests {
    ...
    /// A real 3-share 2-of-3 generation. Words decode and carry x/M/recovery set id.
    ...
    /// The 4-hex recovery set id a card prints for a share.
    ...
    fn renders_each_share_as_its_own_page() {
        let shares = fixture();
        let id = recovery_set_id(&shares[0]);
    ...
    fn recovery_section_points_to_repo_not_inline_instructions() {
        ...
        // The card points to the repo URL only; recovery steps live in the README, not
        // a filename printed on the share.
