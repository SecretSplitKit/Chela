...
pub enum FormatError {
    ...
    MissingWords,
    WordCountMismatch,
    /// The words decoded cleanly but the advisory header disagrees on x/M/recovery set id -
...
impl core::fmt::Display for FormatError {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            ...
            Self::BadIdentifier => f.write_str("the CHELA header's recovery set id is not 4 hex characters"),
...
fn render_readme(
    ...
    let id = format!("{:04X}", first.recovery_set_id & 0x7FF);
...
/// ```
///
/// `<RSID>` is the 11-bit recovery set id (a nonce) in 4 hex digits (high bits zero). The
/// header is advisory: the words alone carry x, M, and the recovery set id. `<N>` is `?` when
...
/// authoritative - they alone carry x, M, and the recovery set id. The header is advisory:
...
pub fn parse_share(header: &str, words_line: &str) -> Result<Share, FormatError> {
    ...
    if (h_nonce & 0x7FF) != share.recovery_set_id || h_x != share.x || h_m != share.threshold {
...
/// authoritative path for words-only backups: the words carry x, M, and the recovery set id,
...
mod tests {
    ...
    /// A real 2-of-3 generation. The words carry x/M/recovery set id; fixtures are never hand-built.
    ...
    fn round_trip_format_then_parse() {
        ...
        assert_eq!(parsed.recovery_set_id, s.recovery_set_id);
    ...
    fn parse_share_rejects_non_ascii_header_without_panicking() {
        // Fuzz crash 8c3bfb86: a 4-byte non-ASCII recovery set id field must not crash the parser.
        let s = &fixture()[0];
        let words = words_line(s);
