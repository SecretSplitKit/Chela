...
pub(crate) fn run_split(kind: SplitKind) -> io::Result<()> {
    ...
    println!(
        "  · {DIM}\"Bitwarden master password; vault lives at https://vaultwarden.homeserver.us\"{RESET}"
    );
    println!(
    ...
    if want_html || want_json {
        let id_hex = format!("{:04X}", shares[0].recovery_set_id & 0x7FF);
...
pub(crate) fn run_recover() -> io::Result<()> {
    ...
    // First share establishes recovery set id / threshold / total / word count; later shares only
    ...
    loop {
        ...
        for s in &from_file {
            let bad = if let Some(first) = expected_first_share.as_ref() {
                first.recovery_set_id != s.recovery_set_id
            ...
            if bad {
                error(&format!(
                    ...
                    expected_first_share.as_ref().unwrap().recovery_set_id & 0x7FF,
                ));
...
impl ParsedHeader {
    fn from_str(s: &str) -> Result<Self, FormatError> {
        ...
        // The recovery set id is 4 hex digits (11-bit value, high bits zero). `from_str_radix`
        // rejects non-ASCII before any byte-indexing, so no char-boundary panic.
        let recovery_set_id =
            u16::from_str_radix(parts[0], 16).map_err(|_| FormatError::BadIdentifier)?;
